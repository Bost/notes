...
∀ (Every) even integer ≧ 2 is a ∑ of two primes
## Riemann hypothesis
The real part of every non-trivial zero of the Zeta function is 1/2 (prime numbers)
## Poincare conjencture
∀ (Every) simply connected, closed 3-manifold is homeomorfic to the 3-sphere (Donuts)
## P vs. NP
∀ problem whose solution can be quickly verified by a computer can also be quickly solved by a computer
## Fermant's Last Theorem
∀ n ≧ 2 there's no such x,y,z that xⁿ + yⁿ = zⁿ
## Continuum Hypothesis
There is no set with cardinality strictly between the cardinalities of integers and real numbers.
