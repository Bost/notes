...
(def ui
  [collapse/ui
   {:id "id0"
    ...
    ;; [:span {:class "m"} ""]
    [:div
     [:div "  Category " [:span {:class "m"} "C = (Obj, Hom, ◦, id)"]]
     ...
     [:div "   - " [:span {:class "m"} "◦"] ": Morphism composition - associative function " [:span {:class "m"} "(f ◦ g) ◦ h = f ◦ (g ◦ h)"]]
     [:div "        " [:span {:class "m"} "Hom(A, B) × Hom(B, C) → Hom(A, C): g ◦ f"] " ; it's a partialy binary operation on Mor(CAT::)"]
     [:div "        " [:span {:class "m"} "(g ◦ f)(x) = g(f(x))"]]
     [:div "   - id: identity morphism on object A: " [:span {:class "m"} "id(A)"]]
     [:div ""]
     ...
     [:div "   Localy Small Category: " [:span {:class "m"} "∀ A,B: Hom(A, B)"] " is a set"]
     [:div ""]
