    ...
    {:id "id1"
     ...
     ;; [:span {:class "m"} ""]
      [:div
       ...
       [:div "    - Operational: \"How it executes\"; reduction relation: e1 -> e2"]
       [:div "    - Denotational: mapping into mathematics; interpretation of terms: ⟦e⟧ = ?;"]
       [:div "      e.g.: " [:span {:class "m"} "⟦ v : τ  ⊢  v : τ ⟧ = idτ "] "- i.e. the meaning of " [:span {:class "m"} "⟦...⟧"] " is an identity on " [:span {:class "m"} "τ"] " i.e. an access to variable " [:span {:class "m"} "v"]]
       [:div ""]
       ...
       [:div "  |-----------------------------------------+--------------------------------------------|"]
       [:div "  | G f :: (a -> b) -> (G a -> G b)         | G f :: (a -> b) -> (G b -> G a)            |"]
       ...
       [:div "         H ◦ α : H ◦ F → H ◦ G"]
       [:div "     is defined as (H ◦ α)A : H(FA) → H(GA) by (H ◦ α)A = H(αA)"]
       [:div ""]
       [:div "***** Left Whiskering"]
       [:div "     If F : C → D and G, H : D → E are Ftors, and if α : G → H is a NaT, the left whiskering"]
       [:div "         α ◦ F : G ◦ F → H ◦ F"]
       [:div "     is defined as (α ◦ F)A : G(FA) → H(FA) by (α ◦ F)A = αFA"]
       [:div ""]
       ...
       [:div "         [(β ◦ G) • (H ◦ α)]A = βGA ◦ H(α A), and"]
       [:div "         [(K ◦ α) • (β ◦ F)]A = K(α A) ◦ βFA"]
       [:div ""]
